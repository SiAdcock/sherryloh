...
jQuery(document).ready(function($) {
  ...
  */
  ...
  //Do testimonials stuff
  if ($('#testimonials-content').length) {
    $.ajax({
      url: '/api/get-subpages.php?page=testimonials',
      dataType: 'json'
    }).done(function(data) {

      var i, max, subpage, wrapper, title, image, content, contentPara; 
      ...
      for (i = 0, max = data.length; i < max; i += 1) {
        ...
        wrapper = $('<div>').addClass('testimonial').addClass('cf');

        title = $('<h3>').html(subpage.title);

        contentPara = $('<p>').html(subpage.content);

        if (subpage.image) {
          content = $('<div>').addClass('m-all').addClass('t-2of3').addClass('d-2of3');
          image =  $('<div>').addClass('m-all').addClass('t-1of3').addClass('d-1of3').addClass('testimonial__img').appendTo(wrapper);

          $('<img>').attr('src', subpage.image)
                    .attr('title', subpage.title)
                    .attr('alt', subpage.title)
                    .appendTo(image);
        } else {
          content = $('<div>').addClass('m-all').addClass('t-3of3').addClass('d-3of3');
        }

        contentPara.appendTo(content);
        title.prependTo(content);
        content.appendTo(wrapper);

        wrapper.appendTo($('#testimonials-content'));

      }
    }).fail(function () {
      var msg = 'Oops, something went wrong. If you have time,'
              + ' please could you drop me an <a href="/contact">email</a>'
              + ' to let me know and I&apos;ll get my IT guy'
              + ' onto it. Many thanks, Sherry :)'
      $('#testimonials-content').html(msg);
    });
  }
